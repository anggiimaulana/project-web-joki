<section class="bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
    <div class="container mx-auto py-8 md:py-12 px-4 sm:px-6 lg:px-8">
        ...
        @if (isset($blog) && $blog->count() > 0)
            <!-- Blog Grid -->
            <div class="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mb-12" id="blog-list">
                ...
                <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-2">
                            {{-- Previous Page Link --}}
                            @if ($blog->onFirstPage())
                                <div class="px-4 py-2 text-gray-400 bg-gray-100 rounded-lg cursor-not-allowed">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M15 19l-7-7 7-7" />
                                    </svg>
                            ...
                            @else
                                <a href="{{ $blog->previousPageUrl() }}"
                                    class="px-4 py-2 text-sky-700 bg-sky-50 hover:bg-sky-100 rounded-lg transition-colors duration-300">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M15 19l-7-7 7-7" />
                                    </svg>
                            ...
                            @foreach ($blog->getUrlRange(1, $blog->lastPage()) as $page => $url)
                                @if ($page == $blog->currentPage())
                                    <div
                                        class="px-4 py-2 bg-gradient-to-r from-sky-600 to-blue-600 text-white rounded-lg font-semibold shadow-md">
                                        {{ $page }}
                                    </div>
                                @else
                                    <a href="{{ $url }}"
                                        class="px-4 py-2 text-gray-700 hover:text-sky-700 hover:bg-sky-50 rounded-lg transition-colors duration-300">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
